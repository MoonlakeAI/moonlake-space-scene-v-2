...
const IMAGE_SIZE = Vector2(660, 660)
...
# Undo history: stores the start index of each stroke in brush_data_list
var stroke_history = []
...
# Control whether painting is enabled (disabled when image is uploaded)
...
func _ready():
	...
	# Enable input processing for editor tools
	set_process_input(true)
	set_process_unhandled_input(true)

	# CRITICAL: Set mouse_filter to allow proper input propagation
	# When painting is disabled, we'll change this to MOUSE_FILTER_IGNORE
	mouse_filter = Control.MOUSE_FILTER_STOP


# Enable painting (allows user to draw)
func enable_painting():
	painting_enabled = true
	# CRITICAL: Allow this control to receive input events
	mouse_filter = Control.MOUSE_FILTER_STOP


# Disable painting (blocks all drawing input)
func disable_painting():
	painting_enabled = false
	# Clear any in-progress stroke state
	mouse_click_start_pos = null
	undo_set = false
	# CRITICAL: Make this control transparent to input - events pass through to nodes behind it
	mouse_filter = Control.MOUSE_FILTER_IGNORE


func _input(event):
	# Handle Ctrl+Z (Windows/Linux) or Cmd+Z (macOS) for undo
	# Allow key repeat so holding the key works at system repeat rate
	if event is InputEventKey and event.pressed:
		# Check for Z key with either Ctrl (Windows/Linux) or Meta/Cmd (macOS)
...
func _process(_delta):
	# Early exit if painting is disabled (e.g., when image is uploaded)
	if not painting_enabled:
		return

	# Use get_global_mouse_position() which works correctly in Window contexts
	var mouse_pos = get_global_mouse_position()

	# Check if the mouse is currently inside the canvas/drawing-area.
	is_mouse_in_drawing_area = false
	var canvas_tl = TL_node.global_position
	var canvas_br = canvas_tl + IMAGE_SIZE
	if mouse_pos.x > canvas_tl.x and mouse_pos.x < canvas_br.x:
		if mouse_pos.y > canvas_tl.y and mouse_pos.y < canvas_br.y:
	...
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
		...
		# Mouse released - stroke is complete
		# Record completed stroke in history if we drew anything
		if undo_set:
			# Record where this stroke started (before any brushes were added)
			stroke_history.append(undo_element_list_num)
		...
		if check_if_mouse_is_inside_canvas():
			...
			if brush_mode == BrushModes.CIRCLE_SHAPE or brush_mode == BrushModes.RECTANGLE_SHAPE:
				var shape_start_index = brush_data_list.size()
				add_brush(mouse_pos, brush_mode)
				# Record the shape stroke in history
				stroke_history.append(shape_start_index)
...
func check_if_mouse_is_inside_canvas():
	...
	if mouse_click_start_pos != null:
		...
		var canvas_tl = TL_node.global_position
		var canvas_br = canvas_tl + IMAGE_SIZE
		if mouse_click_start_pos.x > canvas_tl.x and mouse_click_start_pos.x < canvas_br.x:
			if mouse_click_start_pos.y > canvas_tl.y and mouse_click_start_pos.y < canvas_br.y:
...
func undo_stroke():
	# Remove the last brush/dot if there is one
	if brush_data_list.size() > 0:
		brush_data_list.pop_back()

		# Also remove from stroke history if it was a stroke start point
		if not stroke_history.is_empty() and stroke_history.back() == brush_data_list.size():
...
func add_brush(mouse_pos, type):
	...
	# Convert global mouse position to local coordinates relative to canvas
	var local_pos = mouse_pos - TL_node.global_position
	...
	if type == BrushModes.RECTANGLE_SHAPE:
		var local_start_pos = mouse_click_start_pos - TL_node.global_position
		var TL_pos = Vector2()
		var BR_pos = Vector2()

		# Figure out the left and right positions of the corners and assign them to the proper variable.
		if local_pos.x < local_start_pos.x:
			TL_pos.x = local_pos.x
			BR_pos.x = local_start_pos.x
		else:
			TL_pos.x = local_start_pos.x
			BR_pos.x = local_pos.x

		# Figure out the top and bottom positions of the corners and assign them to the proper variable.
		if local_pos.y < local_start_pos.y:
			TL_pos.y = local_pos.y
			BR_pos.y = local_start_pos.y
		else:
			TL_pos.y = local_start_pos.y
			BR_pos.y = local_pos.y
	...
	if type == BrushModes.CIRCLE_SHAPE:
		...
		var center_pos = Vector2((local_pos.x + local_start_pos.x) / 2, (local_pos.y + local_start_pos.y) / 2)
		...
		new_brush.brush_shape_circle_radius = center_pos.distance_to(Vector2(center_pos.x, local_pos.y))
