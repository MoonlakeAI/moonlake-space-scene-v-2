...
static func render(message: Dictionary) -> Control:
	...
	container.set_meta("message_type", "info")
	...
	# Color icon and text in amber/yellow
	var text_color = Color(1.0, 0.8, 0.3, 1.0)  # Amber/yellow
	...
	var hide_callable = func():
		_hide_horizontal_scrollbar(code_edit)
	code_edit.tree_entered.connect(func():
		hide_callable.call_deferred()
	, CONNECT_ONE_SHOT)

	return container

static func _hide_horizontal_scrollbar(code_edit_node: CodeEdit) -> void:
	for child in code_edit_node.get_children():
