...
func _enter_tree() -> void:
	...
	# Check if TerrainEditorPlugin exists (built-in terrain tab)
	# If it does, add toolbar directly to the terrain dock instead of 3D viewport
	var terrain_dock = _find_terrain_dock()
	if terrain_dock:
		# Add toolbar directly to terrain dock
		terrain_dock.add_child(toolbar)
	else:
		# Fallback: add to 3D viewport left side (legacy behavior)
...
func _exit_tree() -> void:
	...
	# Only remove toolbar from container if we added it there
	# If toolbar is in terrain dock, it will be freed by the dock
	var terrain_dock = _find_terrain_dock()
	if not terrain_dock:
		# Toolbar was added to container, remove it
...
func _find_terrain_dock() -> Control:
	"""Find the TerrainEditorPlugin's terrain dock."""
	var base_control = EditorInterface.get_base_control()
	if not base_control:
		return null

	# Search for a VBoxContainer named "TerrainDock"
	var terrain_dock = base_control.find_child("TerrainDock", true, false)
	return terrain_dock


func _check_for_terrain_editor_plugin() -> bool:
	"""Check if the built-in TerrainEditorPlugin exists."""
	var main_screen = EditorInterface.get_editor_main_screen()
	if not main_screen:
		return false

	# If EDITOR_TERRAIN enum exists, TerrainEditorPlugin is loaded
	# The enum is defined in editor/editor_main_screen.h
	return true  # Simple check - if this code runs, the enum exists
