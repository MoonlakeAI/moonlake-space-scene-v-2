...
func _ready() -> void:
	...
	add_tool_button({ "tool":Terrain3DEditor.SCULPT,
		"add_text":"Smooth (S)", "add_op":Terrain3DEditor.AVERAGE, "add_icon":ICON_HEIGHT_SMOOTH })
	...
	add_tool_button({ "tool":Terrain3DEditor.SCULPT,
		"add_text":"Slope (G)", "add_op":Terrain3DEditor.GRADIENT, "add_icon":ICON_HEIGHT_SLOPE })
	...
	var generate_terrain_button := Button.new()
	generate_terrain_button.set_name("GenerateTerrain")
	generate_terrain_button.set_text("Generate Terrain")
	generate_terrain_button.set_flat(false)
	generate_terrain_button.set_h_size_flags(SIZE_EXPAND_FILL)
	generate_terrain_button.set_icon_alignment(HORIZONTAL_ALIGNMENT_LEFT)
	generate_terrain_button.custom_minimum_size = Vector2(0, 40)
	# Use built-in Godot icon for "Add" or "New"
	if Engine.is_editor_hint():
		var editor_interface = Engine.get_singleton("EditorInterface")
		if editor_interface:
			var base_control = editor_interface.get_base_control()
			if base_control:
				generate_terrain_button.set_button_icon(base_control.get_theme_icon("Add", "EditorIcons"))
	generate_terrain_button.pressed.connect(_on_generate_terrain_pressed)
	add_child(generate_terrain_button)
...
func add_tool_button(p_params: Dictionary) -> void:
	# Create container for this tool (holds both add and sub buttons)
	var tool_container := VBoxContainer.new()
	#tool_container.set_v_size_flags(SIZE_EXPAND_FILL)
	...
	button.set_text(p_params.get("add_text", "blank"))
	...
	if p_params.has("sub_text"):
		...
		button2.set_text(p_params.get("sub_text", "blank"))
		button2.set_flat(false)
		button2.set_toggle_mode(true)
		button2.set_h_size_flags(SIZE_EXPAND_FILL)
		button2.set_v_size_flags(SIZE_EXPAND_FILL)
		button2.set_icon_alignment(HORIZONTAL_ALIGNMENT_LEFT)
		button2.set_button_group(p_params.get("group", sub_tool_group))
		tool_container.add_child(button2)
	else:
...
func _on_generate_terrain_pressed() -> void:
	"""Emit signal when Generate Terrain button is clicked"""
